...
exports.run = async (client, message, args) => {
    ...
    try {
        ...
        console.log(chalk.red('An error occured when running the whitelist command: ' + err));
        return message.channel.send({embed: {
            color: 16733013,
    ...
    message.channel.send({embed: {
        color: 9240450,
        description: `**Success!** whitelisted ${username} for proof ${promoteResponse.newRole.name} (${promoteResponse.newRole.rank})`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        }
    ...
    logchannel.send({embed: {
        color: 2127726,
        description: `<@${message.author.id}> has whitelisted ${username} from ${rankNameInGroup} (${rankInGroup}) to ${promoteResponse.newRole.name} (${promoteResponse.newRole.rank}).`,
        author: {
            name: message.author.tag,
            icon_url: message.author.displayAvatarURL()
        },
